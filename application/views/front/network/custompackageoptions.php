...
<section class="about-text ptb-100">
                ...
                <div class="row">
                    <div class="col-xs-6 col-sm-6 col-md-6">
                        <?php foreach ($membership_options as $options) { ?>
                            <div class="checkbox">
                                <label><input getradiocheck="0" optinval="<?php echo $options['option_price']; ?>" class="optionValue" type="checkbox" value="<?php echo $options['option_id']; ?>" name="<?php echo $options['option_id']; ?>" <?php foreach($choose_option_id as $opttt){ if($options['option_id'] == $opttt){ echo "checked"; } }?>><?php echo $options['option_name']; ?> - $ <?php echo $options['option_price']; ?></label>
                            </div>
                        <?php } ?>
                        <?php foreach ($mem_opt as $opt) { ?>
                            <div class="radio">
                                <label class="radio-inline"><input getradiocheck="1" optinval="<?php echo $opt['option_price']; ?>" class="optionValue" value="<?php echo $opt['option_id']; ?>" type="radio" name="optionradio" <?php foreach($choose_option_id as $opttt){ if($opt['option_id'] == $opttt){ echo "checked"; } }?>><?php echo $opt['option_name']; ?> - $ <?php echo $opt['option_price']; ?></label>
    <!--                                <label><input optinval="<?php echo $options['option_price']; ?>" class="optionValue" type="checkbox" value="<?php echo $options['option_id']; ?>" name="<?php echo $options['option_id']; ?>"><?php echo $options['option_name']; ?> - $ <?php echo $options['option_price']; ?></label>-->
                    ...
                    <div class="col-xs-4 col-sm-4 col-md-4">
                        ...
                        <h3>Total Amount : $ <span id="showamountspan"><?php echo $choose_amount; ?></span></h3>
                         <input type='hidden' name="amount" id="packagetotalamount" value="<?php echo $choose_amount; ?>">
                        <?php } else { ?>
