...
//            }
            ...
            if ($checkpermission->active == 1 || $checkpermission->trial == 1) {
                ...
                //get the posts data
                $data['allbanners'] = $this->BannersModel->getloadbanner($column_name,$config["per_page"], $data['page']);
...
//        }
        ...
        $column_name = "banner_id";
        //get posts data
        $data['allbanners'] = $this->BannersModel->getloadbanner($column_name,$config["per_page"], $offset);
        $option_id = 1;
        $data['favbanner'] = $this->BannersModel->getfavbaninfo($member_id);
        $data['allcountry'] = $this->BannersModel->getAllCountry();
        $data['allcategory'] = $this->BannersModel->getAllCat();
        ...
        if ($makefav) {
            ...
            if ($this->uri->segment('3') == 3) {
             ...
             if ($this->uri->segment('3') == 5) {
                  $this->session->set_flashdata('success', 'PPV adds Favorite Successfully');
                redirect('ppvAddSec');
            }
        } else {
            echo 'error';
