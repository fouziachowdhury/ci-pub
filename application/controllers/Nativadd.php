...
//            }
           ...
           // $hasper = $haspermission[0]['is_active'];
            ...
            if ($checkpermission->active == 1 || $checkpermission->trial == 1) {
                ...
                $column_name = "native_id";
                //call the model function to get the department data
                $data['allnativadds'] = $this->NativaddModel->getAllnativadds($column_name,$config["per_page"], $data['page']);
...
//        echo $totalRec;die;
        ...
        $column_name = "native_id";
        //get posts data
        $data['allnativadds'] = $this->NativaddModel->getAllnativadds($column_name,$config["per_page"], $offset);
        $option_id = 1;
        $data['favbanner'] = $this->NativaddModel->getfavaddsinfo($member_id);
        $data['allcountry'] = $this->BannersModel->getAllCountry();
        $data['allcategory'] = $this->BannersModel->getAllCat();
...
//        print_r($data['allbanners']);die;
 ...
 public function nativTagAutocomplete() {
    ...
    public function nativeCommentModal() {
        $option_id = $_POST['option_id'];
        $addsId = $_POST['addsId'];
//        echo $option_id;die;
        $member_id = $_SESSION['member_id'];
        $data['all_com_info'] = $this->BannersModel->getbannercomments($option_id, $addsId, $member_id);
        $this->load->view('front/netivadds/native_comment_modal', $data);
    }
